...
export default function Strip({
  ...
  return (
    <div style={{
      ...
      justifyContent: 'space-between', padding: '0 44px', flexShrink: 0,
    }}>
      {/* Wordmark */}
      <button
        onClick={() => router.push(`/project/${project.id}`)}
        style={{ fontFamily: 'var(--font-sans)', fontSize: 14, fontWeight: 300, letterSpacing: '0.15em', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--dark)', padding: 0 }}
      >
        proof<span style={{ color: 'var(--mango)' }}>.</span>
      </button>

      {/* Centre — project name + progress marks */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <span style={{ fontSize: 12, color: 'var(--stone)' }}>
          {project.name || 'Untitled'}
        </span>

        {/* Progress marks — only show during a module with questions */}
        {totalCount > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
            {Array.from({ length: totalCount }).map((_, i) => (
              <motion.div
                ...
                animate={{ background: i < answeredCount ? 'var(--mango)' : 'rgba(184,179,172,0.5)' }}
      ...
      {/* Right — notes indicator + ask button + settings */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {thoughtCount > 0 && (
            <motion.button
              ...
              style={{
                ...
                padding: '3px 8px', borderRadius: 20, transition: 'background 0.15s',
              }}
        ...
        <button
          ...
          style={{
            ...
            fontFamily: 'var(--font-sans)', fontSize: 12, color: 'var(--concrete)',
            background: 'none', border: '1px solid rgba(184,179,172,0.6)',
            borderRadius: 20, padding: '5px 14px 5px 9px', cursor: 'pointer', transition: 'all 0.18s',
          }}
